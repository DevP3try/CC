    ...
    function createBinaryParser(child, sep, combine2) {
      return parsimmon_umd_minExports.seqMap(child, parsimmon_umd_minExports.seq(parsimmon_umd_minExports.optWhitespace, sep, parsimmon_umd_minExports.optWhitespace, child).many(), (first, rest) => {
        ...
        for (let index = 1; index < rest.length; index++) {
          node = combine2(node, rest[index][1], rest[index][3]);
        }
        return node;
      });
    }
    ...
    if (recurringValue != "") {
      ...
      if (startDate) {
        rrules.start = new RRule({
          ...options,
          dtstart: (0, import_obsidian4.moment)(startDate).toDate()
        });
      }
      if (dueDate) {
        rrules.due = new RRule({
          ...options,
          dtstart: (0, import_obsidian4.moment)(dueDate).toDate()
        });
      }
      if (scheduledDate) {
        rrules.scheduled = new RRule({
          ...options,
          dtstart: (0, import_obsidian4.moment)(scheduledDate).toDate()
        });
      }
    }
...
var getFilename = (path) => (path.match(/^(?:.*\/)?([^\/]+?|)(?=(?:\.[^\/.]*)?$)/) || [])[1];
var momentToRegex = (momentFormat) => {
...
var getMetaFromNote = (task, metaName, dv) => dv.pages(`"${task.link.path}"`)[metaName][0] || "";
var IsRecurringThisDay = (task, date) => {
  if (!task.recurringValue)
    return false;
  if ((0, import_obsidian4.moment)(date).isBefore(task.moment, "day"))
    return false;
  try {
    let rule = new RRule({
    ...
    let startOfDay = (0, import_obsidian4.moment)(date).startOf("day").toDate();
    let endOfDay = (0, import_obsidian4.moment)(date).endOf("day").toDate();
...
var isDueOrScheduled = (task) => isNotCompletedOrCancelled(task) && (task.due || task.scheduled);
var isTime = (task) => isNotCompletedOrCancelled(task) && hasTimeFormat(task) && isDueOrScheduled(task);
var isTimePassed = (task) => isNotCompletedOrCancelled(task) && isTodayTask(task) && isTimeBeforeCurrentTime(task);
var sortTasks = (a, b, date) => {
  if (a.type == "time" && b.type != "timePassed" && b.type != "time" && !(0, import_obsidian4.moment)(date).isSame((0, import_obsidian4.moment)(), "day")) {
    return 2 - b.typePriority;
  }
  if (b.type == "time" && a.type != "timePassed" && a.type != "time" && !(0, import_obsidian4.moment)(date).isSame((0, import_obsidian4.moment)(), "day")) {
    return a.typePriority - 2;
  }
  if (a.typePriority != b.typePriority && !((a.type == "timePassed" && b.type == "time" || b.type == "timePassed" && a.type == "time") && !(0, import_obsidian4.moment)(date).isSame((0, import_obsidian4.moment)(), "day"))) {
...
var calculatePagePoints = (page) => {
  const calculateTaskPoints = (task, pageMulti2) => 10 * (task["\u2733\uFE0F"] !== void 0 ? task["\u2733\uFE0F"] : 1) * pageMulti2;
  const pageMulti = page["\u2733\uFE0F"] !== void 0 ? page["\u2733\uFE0F"] : 1;
  return page.file.tasks.where((t) => t.completed && t.fullyCompleted).values.reduce(
    (acc, task) => acc + calculateTaskPoints(task, pageMulti),
    0
  );
};
var calculateTotalPoints = (dv) => dv.pages().values.reduce(
  (acc, page) => acc + calculatePagePoints(page),
  0
);
var currentLevelBasedOnXp = (totalXp, x, y) => {
  let level = 0;
  let xpNeeded = 0;
...
var xpForNextLevel = (currentLevel, x, y) => Math.pow(x * (currentLevel + 1), y);
var totalXpToTargetLevel = (targetLevel, x, y) => {
  let totalXp = 0;
...
var EngageCurrentFile = class {
  constructor(app) {
    ...
    this.setFocus = () => {
      var _a, _b;
      if (this.dv.pages().where((p) => {
        var _a2, _b2;
        return ((_a2 = p["Handled By"]) == null ? void 0 : _a2.path) == ((_b2 = this.app.workspace.getActiveFile()) == null ? void 0 : _b2.path) && !p.Archived;
      }).length > 0)
        this.focus = this.dv.page(this.app.workspace.getActiveFile().path);
      if (!this.focus && this.dv.page((_a = this.app.workspace.getActiveFile()) == null ? void 0 : _a.path)["Handled By"])
        this.focus = this.dv.page(this.dv.page((_b = this.app.workspace.getActiveFile()) == null ? void 0 : _b.path)["Handled By"].path);
      if (this.focus)
...
var XpFeedback = class {
  constructor(app) {
    this.displayXpEarned = (file, callBack) => {
        ...
        for (let task of fileTasks) {
          ...
          if (task.dueDate && isBefore) {
            ...
            if (Object.keys(task.rrules).length > 0) {
              ...
              const nextDueDate = task.dueDate ? (0, import_obsidian11.moment)(task.rrules.due.after((0, import_obsidian11.moment)(task.dueDate).toDate())) : void 0;
              const nextScheduledDate = task.scheduledDate ? (0, import_obsidian11.moment)(task.rrules.scheduled.after((0, import_obsidian11.moment)(task.scheduledDate).toDate())) : void 0;
              const nextStartDate = task.startDate ? (0, import_obsidian11.moment)(task.rrules.start.after((0, import_obsidian11.moment)(task.startDate).toDate())) : void 0;
              if (nextDueDate) {
                if (nextDueDate.isAfter((0, import_obsidian11.moment)(task.dueDate))) {
              ...
              if (nextScheduledDate) {
                if (nextScheduledDate.isAfter((0, import_obsidian11.moment)(task.scheduledDate))) {
              ...
              if (nextStartDate) {
                if (nextStartDate.isAfter((0, import_obsidian11.moment)(task.startDate))) {
  ...
  constructor(app) {
    ...
    this.onChooseItem = (item, _) => {
      ...
      this.app.vault.process(this.app.workspace.getActiveFile(), (content) => {
        ...
        let oldestDate = (0, import_obsidian12.moment)();
        for (const match of newContent.matchAll(taskRegex)) {
          const task = match[0];
          const dates = task.match(dateRegex);
          if (dates && dates.length > 0)
            for (const currentDate of dates) {
              if ((0, import_obsidian12.moment)(currentDate.split(" ")[1]).isBefore(oldestDate))
                oldestDate = (0, import_obsidian12.moment)(currentDate.split(" ")[1]);
            }
        }
        let diff = (0, import_obsidian12.moment)().diff(oldestDate, "days");
        const startDate = (0, import_obsidian12.moment)(oldestDate);
        const endDate = (0, import_obsidian12.moment)(oldestDate);
        if (item.value == 0) {
        ...
        diff += getRandomTime(startDate, endDate).diff((0, import_obsidian12.moment)(), "days");
        for (const match of newContent.matchAll(taskRegex)) {
          const task = match[0];
          const dates = task.match(dateRegex);
          if (dates && dates.length > 0)
            for (const currentDate of dates)
              newContent = newContent.replace(currentDate, `${currentDate.split(" ")[0]} ${(0, import_obsidian12.moment)(currentDate.split(" ")[1]).add(diff, "days").format("YYYY-MM-DD")}`);
        }
        return newContent;
      });
    };
...
// src/View/ProfileView.ts
var import_obsidian15 = require("obsidian");
var import_obsidian_dataview7 = __toESM(require_lib());
var ProfileView = class extends import_obsidian15.ItemView {
  constructor(leaf, plugin) {
    ...
    this.display = () => {
      const { contentEl } = this;
      const mastery = () => this.dv.pages('"3.Competences" AND (#Competence)').filter((p) => p.Level !== void 0).array().reduce((sum, p) => sum + p.Level, 0);
      contentEl.empty();
      let imgURL = "";
      const imgPath = this.app.vault.getFileByPath(`${this.plugin.settings.configFolder}/Avatar.png`);
      if (imgPath instanceof import_obsidian15.TFile)
    ...
    this.plugin = plugin;
    this.dv = (0, import_obsidian_dataview7.getAPI)(plugin.app);
  }
  getViewType() {
    return "Profile View";
  }
...
var import_obsidian_dataview8 = __toESM(require_lib());
...
var cellTemplate = (cls, weekday, dailyNote, cellName, cellContent) => `<div class='cell ${cls}' data-weekday='${weekday}'><a class='internal-link cellName' href='${dailyNote}'>${cellName}</a><div class='cellContent'>${cellContent}</div></div>`;
var taskTemplate = (taskPath, cls, style, title, note, icon, relative, taskContent) => `<a class='internal-link' href='${taskPath}'><div class='task ${cls}' style='${style}' title='${title}'><div class='inner'><div class='note'>${note}</div><div class='icon'>${icon}</div><div class='description' data-relative='${relative}'>${taskContent}</div></div></div></a>`;
var taskNumberTemplate = (number, cls) => `<div class='task taskNumber ${cls}' title='${number}'>${number}</div>`;
var capitalize = (str) => str[0].toUpperCase() + str.slice(1);
var getCurrent = () => ({
  today: (0, import_obsidian16.moment)().format("YYYY-MM-DD"),
  day: (0, import_obsidian16.moment)().format("d"),
  month: (0, import_obsidian16.moment)().format("M"),
  year: (0, import_obsidian16.moment)().format("YYYY")
});
var getMeta = (dailyNoteFormat, globalTaskFilter, disableRecurrence, tasks) => {
  if (!dailyNoteFormat)
  ...
  for (let task of tasks) {
    ...
    let dailyNoteMatch = taskFile == null ? void 0 : taskFile.match(dailyNoteRegEx);
    let dailyTaskMatch = taskText.match(/(\d{4}\-\d{2}\-\d{2})/);
    let repeatMatch = taskText.includes("\u{1F501}");
    ...
    if (isOverdue(task)) {
      task.type = "overdue";
      task.typePriority = 1;
      task.moment = task.due ? (0, import_obsidian16.moment)(task.due) : (0, import_obsidian16.moment)(task.scheduled);
      ...
      if (task.due && task.scheduled && (0, import_obsidian16.moment)(task.due).isSameOrBefore(task.scheduled) || task.due) {
        task.type = "due";
        task.typePriority = 3;
        task.moment = (0, import_obsidian16.moment)(task.due);
      } else if (task.scheduled) {
        task.type = "scheduled";
        task.typePriority = 3;
        task.moment = (0, import_obsidian16.moment)(task.scheduled);
      }
    } else if (isStart(task) && !hasTimeFormat(task)) {
      task.type = "start";
      task.typePriority = 4;
      task.moment = (0, import_obsidian16.moment)(task.start);
    } else if (isTime(task)) {
      task.type = "time";
      task.typePriority = 5;
      let time = getTime(taskText).split(":");
      task.moment = task.due ? (0, import_obsidian16.moment)(task.due).startOf("day") : (0, import_obsidian16.moment)(task.scheduled).startOf("day");
      task.moment.add(parseInt(time[0]), "hours").add(parseInt(time[1]), "minutes");
    } else if (isDone(task)) {
      task.type = "done";
      task.typePriority = 6;
      task.moment = (0, import_obsidian16.moment)(task.completion);
    } else if (isCancelled(task)) {
      task.type = "cancelled";
      task.typePriority = 7;
      if (task.cancelled)
        task.moment = (0, import_obsidian16.moment)(task.cancelled);
    }
    if (dailyNoteMatch && !dailyTaskMatch) {
      task.dailyNote = (0, import_obsidian16.moment)(dailyNoteMatch[1], dailyNoteFormat).format("YYYY-MM-DD");
    }
    if (task.type != "done" && task.type != "cancelled" && repeatMatch) {
      ...
      task.text = task.text.substring(0, taskText.indexOf("\u{1F501}"));
    }
    ;
    if (taskText.includes("\u{1F53A}")) {
      ...
      task.priority = 1;
    } else if (taskText.includes("\u{1F53C}")) {
      task.priority = 2;
...
var getTasks = (tasks, date) => tasks.filter((t) => filterTasks(t, date)).sort((t) => t, "asc", (a, b) => sortTasks(a, b, date));
var isOverdue = (task) => isNotCompletedOrCancelled(task) && (task.due && (0, import_obsidian16.moment)(task.due).isBefore((0, import_obsidian16.moment)(), "day") || task.scheduled && (0, import_obsidian16.moment)(task.scheduled).isBefore((0, import_obsidian16.moment)(), "day"));
var isStart = (task) => isNotCompletedOrCancelled(task) && task.start;
var isDone = (task) => task.completed && task.checked;
var isCancelled = (task) => !task.completed && task.checked;
var setTaskContentContainer = (status, dv, date, skipRecurrences) => {
  let cellContent = "";
  for (let task of status) {
    if (skipRecurrences && task.recurrence)
      continue;
...
var updateCounters = (status, counter, currentDate) => {
  for (let key in counter)
    if (key != "overdue")
      counter[key] += status.filter((t) => t.type == key).length;
  counter.recurrence += status.filter((t) => t.recurrence && !(t.type == "overdue" && (0, import_obsidian16.moment)().isSame(currentDate, "day"))).length;
  counter.overdue += status.filter((t) => t.type == "overdue" && (0, import_obsidian16.moment)().isSame(currentDate, "day")).length;
};
var setStatisticPopUp = (rootNode) => {
...
var setWeekViewContext = (rootNode) => {
...
var setStatisticValues = (rootNode, counter) => {
  ...
  if (rootNode.querySelector("#statisticDone"))
    rootNode.querySelector("#statisticDone").innerText = `\u2705 Done: ${done}/${taskCounter}`;
  if (rootNode.querySelector("#statisticDue"))
    rootNode.querySelector("#statisticDue").innerText = `\u{1F4C5} Due: ${due}`;
  if (rootNode.querySelector("#statisticOverdue"))
    rootNode.querySelector("#statisticOverdue").innerText = `\u26A0\uFE0F Overdue: ${overdue}`;
  if (rootNode.querySelector("#statisticStart"))
    rootNode.querySelector("#statisticStart").innerText = `\u{1F6EB} Start: ${start}`;
  if (rootNode.querySelector("#statisticScheduled"))
    rootNode.querySelector("#statisticScheduled").innerText = `\u23F3 Scheduled: ${scheduled}`;
  if (rootNode.querySelector("#statisticRecurrence"))
    rootNode.querySelector("#statisticRecurrence").innerText = `\u{1F501} Recurrence: ${recurrence}`;
  if (rootNode.querySelector("#statisticDailyNote"))
    rootNode.querySelector("#statisticDailyNote").innerText = `\u{1F4C4} Daily Notes: ${dailyNote}`;
};
var removeExistingView = (rootNode) => {
...
var filterTasks = (task, date) => {
  if (task.type == "overdue" && (0, import_obsidian16.moment)(date).isSame((0, import_obsidian16.moment)(), "day")) {
    return true;
  } else if (task.type == "overdue" && (0, import_obsidian16.moment)(date).isBefore((0, import_obsidian16.moment)(), "day")) {
    return false;
  }
  if (task.recurrence) {
    return IsRecurringThisDay(task, date);
  } else if (task.moment) {
    return task.moment.isSame((0, import_obsidian16.moment)(date), "day");
  } else {
    return false;
  }
};
var setTask = (obj, cls, dv) => {
  const lighter = 25;
  const darker = -40;
  ...
  const relative = obj.due ? (0, import_obsidian16.moment)(obj.due).fromNow() : "";
  ...
  if (cls.toLocaleLowerCase() == "done")
    ...
    taskIcon = "\u{1F4C4}";
  else if (cls.toLocaleLowerCase() == "time" || cls.toLocaleLowerCase() == "timepassed" && !obj.moment.isSame((0, import_obsidian16.moment)(), "day"))
    taskIcon = "\u231A";
...
var setStatisticPopUpEvents = (rootNode) => {
...
var setWeekViewContextEvents = (rootNode) => {
  ...
  constructor(app) {
    ...
    this.setTasks = (pages, hideFileWithProps) => {
      ...
      if (pages == "") {
        ...
        const pagesContent = (_a = pages.match(/\((.*)\)/)) == null ? void 0 : _a[1];
        if (pagesContent) {
      ...
      if (hideFileWithProps) {
        this.tasks = dvPages.flatMap((p) => {
          let tasks = p.file.tasks;
          hideFileWithProps.split(",").forEach((prop) => {
            if (p[prop.trim()])
    ...
    this.displayCalendar = (input, element) => {
      ...
      if (view.toLowerCase() == "month") {
        this.selectedDate = (0, import_obsidian17.moment)(startPosition || (0, import_obsidian17.moment)(), "YYYY-MM").date(1);
      } else if (view.toLowerCase() == "week") {
        this.selectedDate = (0, import_obsidian17.moment)(startPosition || (0, import_obsidian17.moment)(), "YYYY-ww").startOf("week");
    ...
    this.setButtonEvents = () => {
      this.rootNode.querySelectorAll("button").forEach((btn) => btn.addEventListener("click", () => {
        ...
        if (btn.className == "previous") {
          if (activeView == "month") {
            this.selectedDate = (0, import_obsidian17.moment)(this.selectedDate).subtract(1, "months");
            this.getMonth();
          } else if (activeView == "week") {
            this.selectedDate = (0, import_obsidian17.moment)(this.selectedDate).subtract(7, "days").startOf("week");
            this.getWeek();
          ...
          if (activeView == "month") {
            this.selectedDate = (0, import_obsidian17.moment)().date(1);
            this.getMonth();
          } else if (activeView == "week") {
            this.selectedDate = (0, import_obsidian17.moment)().startOf("week");
            this.getWeek();
          } else if (activeView == "list") {
            this.selectedDate = (0, import_obsidian17.moment)().date(1);
            this.getList();
          ...
          if (activeView == "month") {
            this.selectedDate = (0, import_obsidian17.moment)(this.selectedDate).add(1, "months");
            this.getMonth();
          } else if (activeView == "week") {
            this.selectedDate = (0, import_obsidian17.moment)(this.selectedDate).add(7, "days").startOf("week");
            this.getWeek();
          } else if (activeView == "list") {
            this.selectedDate = (0, import_obsidian17.moment)(this.selectedDate).add(1, "months");
            this.getList();
          ...
          if ((0, import_obsidian17.moment)().format("ww-YYYY") == (0, import_obsidian17.moment)(this.selectedDate).format("ww-YYYY")) {
            this.selectedDate = (0, import_obsidian17.moment)().date(1);
          } else {
            this.selectedDate = (0, import_obsidian17.moment)(this.selectedDate).date(1);
          }
          ;
          this.getMonth();
          ...
          if (this.rootNode.getAttribute("view") == "week") {
            let leftPos = this.rootNode.querySelector("button.weekView").offsetLeft;
            this.rootNode.querySelector(".weekViewContext").style.left = leftPos + "px";
            ...
            if ((0, import_obsidian17.moment)().format("MM-YYYY") != (0, import_obsidian17.moment)(this.selectedDate).format("MM-YYYY")) {
              this.selectedDate = (0, import_obsidian17.moment)(this.selectedDate).startOf("month").startOf("week");
            } else {
              this.selectedDate = (0, import_obsidian17.moment)().startOf("week");
            }
            ;
    ...
    this.getMonth = async () => {
      ...
      this.rootNode.querySelector("button.current").innerHTML = `<span>${(0, import_obsidian17.moment)(this.selectedDate).format("MMMM")}</span><span>${(0, import_obsidian17.moment)(this.selectedDate).format("YYYY")}</span>`;
      let firstDayOfMonth = parseInt((0, import_obsidian17.moment)(this.selectedDate).format("d"));
      let lastDateOfMonth = parseInt((0, import_obsidian17.moment)(this.selectedDate).endOf("month").format("D"));
      let counter = {
        due: 0,
        done: 0,
        overdue: 0,
      ...
      let monthName = (0, import_obsidian17.moment)(this.selectedDate).format("MMM").replace(".", "").substring(0, 3);
      let current = getCurrent();
      if (firstDayOfMonth == 0)
        firstDayOfMonth = 7;
      let gridHeads = "";
      for (let h = 0 - firstDayOfMonth + this.firstDayOfWeek; h < 7 - firstDayOfMonth + this.firstDayOfWeek; h++) {
        let weekDayNr = (0, import_obsidian17.moment)(this.selectedDate).add(h, "days").format("d");
        let weekDayName = (0, import_obsidian17.moment)(this.selectedDate).add(h, "days").format("ddd");
        if (current.day == weekDayNr && current.month == (0, import_obsidian17.moment)(this.selectedDate).format("M") && current.year == (0, import_obsidian17.moment)(this.selectedDate).format("YYYY")) {
      ...
      for (let w = 1; w < 7; w++) {
        tasksPromises.push((async () => {
          ...
          for (let i = starts; i < starts + 7; i++) {
            if (i == starts) {
              weekNr = (0, import_obsidian17.moment)(this.selectedDate).add(i, "days").format("w");
              yearNr = (0, import_obsidian17.moment)(this.selectedDate).add(i, "days").format("YYYY");
            }
            ;
            let currentDate = (0, import_obsidian17.moment)(this.selectedDate).add(i, "days").format("YYYY-MM-DD");
            let dailyNotePath = this.dailyNoteFolder + "/" + currentDate;
            let weekDay = (0, import_obsidian17.moment)(this.selectedDate).add(i, "days").format("d");
            let shortDayName = (0, import_obsidian17.moment)(this.selectedDate).add(i, "days").format("D");
            let longDayName = (0, import_obsidian17.moment)(this.selectedDate).add(i, "days").format("D. MMM");
            let status = getTasks(this.tasks, currentDate);
            if ((0, import_obsidian17.moment)(this.selectedDate).format("MM") == (0, import_obsidian17.moment)(this.selectedDate).add(i, "days").format("MM")) {
            ...
            if (parseInt((0, import_obsidian17.moment)(this.selectedDate).add(i, "days").format("D")) == 1) {
    ...
    this.getWeek = async () => {
      ...
      if (this.rootNode.querySelector("button.current"))
        this.rootNode.querySelector("button.current").innerHTML = `<span>${(0, import_obsidian17.moment)(this.selectedDate).format("YYYY")}</span><span>${(0, import_obsidian17.moment)(this.selectedDate).format("[W]w")}</span>`;
      let gridContent = "";
      let currentWeekday = parseInt((0, import_obsidian17.moment)(this.selectedDate).format("d"));
      let weekNr = (0, import_obsidian17.moment)(this.selectedDate).format("[W]w");
      let counter = {
        due: 0,
        done: 0,
        overdue: 0,
      ...
      for (let i = 0 - currentWeekday + this.firstDayOfWeek; i < 7 - currentWeekday + this.firstDayOfWeek; i++) {
        tasksPromises.push((async () => {
          ...
          let dailyNotePath = this.dailyNoteFolder + "/" + currentDate;
          let weekDay = (0, import_obsidian17.moment)(this.selectedDate).add(i, "days").format("d");
          let dayName = (0, import_obsidian17.moment)(currentDate).format("ddd D.");
          let longDayName = (0, import_obsidian17.moment)(currentDate).format("ddd, D. MMM");
    ...
    this.getList = async () => {
      ...
      let monthName = (0, import_obsidian17.moment)(this.selectedDate).format("MMM").replace(".", "").substring(0, 3);
      const tasksPromises = [];
      for (let i = 0; i < parseInt((0, import_obsidian17.moment)(this.selectedDate).endOf("month").format("D")); i++) {
        tasksPromises.push((async () => {
          let currentDate = (0, import_obsidian17.moment)(this.selectedDate).startOf("month").add(i, "days").format("YYYY-MM-DD");
          let status = getTasks(this.tasks, currentDate);
          updateCounters(status, counter, currentDate);
          if ((0, import_obsidian17.moment)().format("YYYY-MM-DD") == currentDate) {
            let overdueDetails = `<details open class='overdue'><summary>Overdue</summary>${setTaskContentContainer(status.filter((t) => t.type == "overdue"), this.dv, currentDate)}</details>`;
            let todayDetails = `<details open class='today'><summary>Today</summary>${setTaskContentContainer(status.filter((t) => t.type != "overdue"), this.dv, currentDate)}</details>`;
            let upcomingContent = "";
            for (let t = 1; t < this.upcomingDays + 1; t++) {
              let next = (0, import_obsidian17.moment)(currentDate).add(t, "days").format("YYYY-MM-DD");
            ...
            listContent += `<details open class='today'><summary><span>${(0, import_obsidian17.moment)(currentDate).format("dddd, D")}</span><span class='weekNr'>${(0, import_obsidian17.moment)(currentDate).format("[W]w")}</span></summary><div class='content'>${overdueDetails}${todayDetails}${upcomingDetails}</div></details>`;
          } else {
            listContent += `<details open><summary><span>${(0, import_obsidian17.moment)(currentDate).format("dddd, D")}</span><span class='weekNr'>${(0, import_obsidian17.moment)(currentDate).format("[W]w")}</span></summary><div class='content'>${setTaskContentContainer(status, this.dv, currentDate)}</div></details>`;
          }
        })());
      }
      await Promise.all(tasksPromises);
      ...
      if ((0, import_obsidian17.moment)().format("YYYY-MM") == (0, import_obsidian17.moment)(this.selectedDate).format("YYYY-MM")) {
        let listElement = this.rootNode.querySelector(".list");
        let todayElement = this.rootNode.querySelector(".today");
    ...
    this.setWrapperEvents = () => {
      this.rootNode.querySelectorAll(".wrapperButton").forEach((wBtn) => wBtn.addEventListener("click", () => {
        ...
        this.selectedDate = (0, import_obsidian17.moment)((0, import_obsidian17.moment)(year).add(week - 1, "weeks")).startOf("week");
        this.rootNode.querySelector(`.grid`).remove();
        this.getWeek();
      }));
    };
    this.dv = (0, import_obsidian_dataview8.getAPI)(app);
  }
};

// src/Component/Timeline.ts
var import_obsidian19 = require("obsidian");
var import_obsidian_dataview9 = __toESM(require_lib());
...
var getMeta2 = (tasks, input) => {
  let { inbox, taskOrder, globalTaskFilter, dailyNoteFormat, done, forward, disableRecurrence, numberOfDays } = input;
  if (!dailyNoteFormat)
    dailyNoteFormat = "YYYY-MM-DD";
  let today = (0, import_obsidian18.moment)().format("YYYY-MM-DD");
  let dailyNoteRegEx = momentToRegex(dailyNoteFormat);
  let timelineDates = [];
  numberOfDays = numberOfDays || "1";
  for (let i = 0; i < parseInt(numberOfDays); i++) {
    timelineDates.push((0, import_obsidian18.moment)(today).add(i, "days").format("YYYY-MM-DD"));
  }
  ...
  for (let task of tasks) {
    ...
    if (dailyNoteMatch && task.completed == false && task.checked == false) {
      task.dailyNote = true;
      if (!dailyTaskMatch) {
        if ((0, import_obsidian18.moment)(dailyNoteMatch[1], dailyNoteFormat).format("YYYY-MM-DD") < today) {
          if (forward == true) {
            happens["unplanned"] = today;
            task.order = taskOrder.indexOf("unplanned");
          } else {
            happens["unplanned"] = (0, import_obsidian18.moment)(dailyNoteMatch[1], dailyNoteFormat).format("YYYY-MM-DD");
            task.order = taskOrder.indexOf("unplanned");
          }
          ;
        } else {
      ...
      ;
    } else if (dailyNoteMatch && task.completed == false && task.checked == true && (0, import_obsidian18.moment)(dailyNoteMatch[1], dailyNoteFormat).format("YYYY-MM-DD") >= today) {
      happens["cancelled"] = (0, import_obsidian18.moment)(dailyNoteMatch[1], dailyNoteFormat).format("YYYY-MM-DD");
    ...
    while (inlineFields = /\[([^\]]+)\:\:([^\]]+)\]/g.exec(task.text)) {
      ...
      if (fieldKey == "due" || fieldKey == "scheduled" || fieldKey == "start" || fieldKey == "completion") {
        let fieldDate = (0, import_obsidian18.moment)(fieldValue).format("YYYY-MM-DD");
    ...
    if (dueMatch && task.completed == false && task.checked == false) {
      task.text = task.text.replace(dueMatch[0], "");
      task.moment = (0, import_obsidian18.moment)(dueMatch[1]);
      ...
      if (dueMatch[1] < today) {
        ...
        task.order = taskOrder.indexOf("due");
      } else if (dueMatch[1] > (0, import_obsidian18.moment)().format("YYYY-MM-DD")) {
        happens["due"] = dueMatch[1];
        task.order = taskOrder.indexOf("due");
      }
      ;
    ...
    if (scheduledMatch && task.completed == false && task.checked == false) {
      task.moment = (0, import_obsidian18.moment)(scheduledMatch[1]);
      task.text = task.text.replace(scheduledMatch[0], "");
    ...
    if (startMatch && task.completed == false && task.checked == false) {
      task.moment = (0, import_obsidian18.moment)(startMatch[1]);
      task.text = task.text.replace(startMatch[0], "");
    ...
    ;
    let lowMatch = taskText.includes("\u{1F53D}");
    if (lowMatch) {
      task.text = task.text.replace("\u{1F53D}", "");
      task.priority = "E";
      task.priorityLabel = "low priority";
    }
    ;
    let mediumMatch = taskText.includes("\u{1F53C}");
    if (mediumMatch) {
      task.text = task.text.replace("\u{1F53C}", "");
      task.priority = "C";
    ...
    ;
    let highestMatch = taskText.includes("\u{1F53A}");
    if (highestMatch) {
      task.text = task.text.replace("\u{1F53A}", "");
      task.priority = "A";
...
var getRelative = (someDate) => {
  let date = (0, import_obsidian18.moment)(someDate);
  if ((0, import_obsidian18.moment)().diff(date, "days") >= 1 || (0, import_obsidian18.moment)().diff(date, "days") <= -1) {
    return date.fromNow();
  } else {
    return date.calendar().split(" ")[0];
  }
};
var getSelectOptions = (rootNode, dailyNoteFolder, dailyNoteFormat, taskFiles, inbox, select) => {
  if (!dailyNoteFormat)
    dailyNoteFormat = "YYYY-MM-DD";
  const currentDailyNote = dailyNoteFolder + (0, import_obsidian18.moment)().format(dailyNoteFormat) + ".md";
  ...
  taskFiles.forEach(function(file) {
    ...
    let parentFolder = file.split("/")[file.split("/").length - 2] == null ? "" : `${secondParentFolder}\u{1F4C2}&nbsp;${file.split("/")[file.split("/").length - 2]} / `;
    let filePath = `${parentFolder}\u{1F4C4}&nbsp;${getFilename(file)}`;
...
var addNewTask = (fileText, newTask, section) => {
...
var Timeline = class {
  constructor(app) {
    this.checkErrors = (input) => {
      ...
      if (!pages && pages != "") {
        this.rootNode.createSpan('> [!ERROR] Missing pages parameter\n> \n> Please set the pages parameter like\n> \n> `pages: ""`');
        return true;
      }
      ;
      if (dailyNoteFormat) {
        if (((_a = dailyNoteFormat.match(/[|\\YMDWwd.,-: \[\]]/g)) == null ? void 0 : _a.length) != dailyNoteFormat.length) {
          this.rootNode.createSpan("> [!ERROR] The `dailyNoteFormat` contains invalid characters");
          return true;
        }
      }
      ;
    ...
    this.createTimeline = (input, el) => {
      ...
      this.rootNode.createSpan();
      if (this.checkErrors(input))
        return;
      this.setTasks(pages, hideFileWithProps);
      if (!taskFiles) {
        this.taskFiles = [...new Set(this.dv.pages().file.map((f) => f.tasks.filter((t) => !t.completed)).path)].sort();
      } else {
      ...
      this.timelineDates = getMeta2(this.tasks, input);
    ...
    this.setEvents = () => {
      ...
      (_a = this.rootNode.querySelector(".todayHeader")) == null ? void 0 : _a.addEventListener("click", () => {
        this.rootNode.classList.toggle("todayFocus");
      });
      this.rootNode.querySelectorAll(".task:not(.star, .add)").forEach((t) => t.addEventListener("click", (e) => {
        var _a2, _b2, _c2, _d2;
        let link = t.getAttribute("data-link") || "";
        let line = t.getAttribute("data-line") || "";
        let col = t.getAttribute("data-col") || "";
        if ((_a2 = e.target) == null ? void 0 : _a2.closest(".task .tag")) {
        } else if ((_b2 = e.target) == null ? void 0 : _b2.closest(".timeline .icon")) {
          let task = (_c2 = e.target) == null ? void 0 : _c2.closest(".task");
          let icon = (_d2 = e.target) == null ? void 0 : _d2.closest(".timeline .icon");
          if (task)
      ...
      (_b = this.rootNode.querySelector(".ok")) == null ? void 0 : _b.addEventListener("click", () => {
        var _a2, _b2;
        let filePath = (_a2 = this.rootNode.querySelector(".fileSelect")) == null ? void 0 : _a2.value;
        let newTask = (_b2 = this.rootNode.querySelector(".newTask")) == null ? void 0 : _b2.value;
        if (newTask.length > 1) {
          try {
            let abstractFilePath = this.app.vault.getAbstractFileByPath(filePath);
            if (abstractFilePath && abstractFilePath instanceof import_obsidian19.TFile) {
              this.app.vault.process(abstractFilePath, (fileText) => addNewTask(fileText, newTask, this.section));
            } else {
              this.app.vault.create(filePath, "- [ ] " + newTask);
            }
            ;
            new import_obsidian19.Notice("New task saved!");
            if (this.rootNode.querySelector(".newTask") != null) {
              this.rootNode.querySelector(".newTask").value = "";
              this.rootNode.querySelector(".newTask").focus();
            }
          } catch (err) {
            new import_obsidian19.Notice("Something went wrong!");
          }
          ;
        } else {
          this.rootNode.querySelector(".newTask").focus();
      ...
      (_c = this.rootNode.querySelector(".fileSelect")) == null ? void 0 : _c.addEventListener("change", () => {
        this.rootNode.querySelector(".newTask").focus();
      });
      this.rootNode.querySelector(".newTask").addEventListener("input", () => {
        let input = this.rootNode.querySelector(".newTask");
        let newTask = input.value;
        if (newTask.includes("due ")) {
          input.value = newTask.replace("due", "\u{1F4C5}");
        }
        ;
        if (newTask.includes("start ")) {
          input.value = newTask.replace("start", "\u{1F6EB}");
        }
        ;
        ...
        if (newTask.includes("medium ")) {
          input.value = newTask.replace("medium", "\u{1F53C}");
        }
        ;
        if (newTask.includes("low ")) {
          input.value = newTask.replace("low", "\u{1F53D}");
        }
        ;
        if (newTask.includes("repeat ")) {
          input.value = newTask.replace("repeat", "\u{1F501}");
        }
        ;
        if (newTask.includes("recurring ")) {
          input.value = newTask.replace("recurring", "\u{1F501}");
        }
        ;
        if (newTask.includes("today ")) {
          input.value = newTask.replace("today", (0, import_obsidian19.moment)().format("YYYY-MM-DD"));
        }
        ;
        if (newTask.includes("tomorrow ")) {
          input.value = newTask.replace("tomorrow", (0, import_obsidian19.moment)().add(1, "days").format("YYYY-MM-DD"));
        }
        ;
        if (newTask.includes("yesterday ")) {
          input.value = newTask.replace("yesterday", (0, import_obsidian19.moment)().subtract(1, "days").format("YYYY-MM-DD"));
        }
        ;
        ...
        if (futureDate) {
          ...
          let date = (0, import_obsidian19.moment)().add(x, unit).format("YYYY-MM-DD[ ]");
        ...
        if (weekday) {
          ...
          if ((0, import_obsidian19.moment)().isoWeekday() < dayINeed) {
            input.value = newTask.replace(weekday[1], (0, import_obsidian19.moment)().isoWeekday(dayINeed).format("YYYY-MM-DD"));
          } else {
            input.value = newTask.replace(weekday[1], (0, import_obsidian19.moment)().add(1, "weeks").isoWeekday(dayINeed).format("YYYY-MM-DD"));
          }
        }
        this.rootNode.querySelector(".newTask").focus();
      });
      (_d = this.rootNode.querySelector(".newTask")) == null ? void 0 : _d.addEventListener("keyup", (e) => {
        var _a2;
        if (e.key === "Enter") {
          (_a2 = this.rootNode.querySelector(".ok")) == null ? void 0 : _a2.click();
        }
        ;
      });
      (_e = this.rootNode.querySelector(".newTask")) == null ? void 0 : _e.addEventListener("focus", () => {
        var _a2;
        (_a2 = this.rootNode.querySelector(".quickEntryPanel")) == null ? void 0 : _a2.classList.add("focus");
      });
      (_f = this.rootNode.querySelector(".newTask")) == null ? void 0 : _f.addEventListener("blur", () => {
        var _a2;
        (_a2 = this.rootNode.querySelector(".quickEntryPanel")) == null ? void 0 : _a2.classList.remove("focus");
      });
    };
    this.openFile = (link, line, col) => {
      this.app.workspace.openLinkText("", link).then(() => {
        if (line && col) {
          try {
            ...
            let cmEditor = this.app.workspace.getActiveViewOfType(import_obsidian19.MarkdownView).editor;
    ...
    this.getTimeline = (dateFormat) => {
      ...
      for (let timelineDate of this.timelineDates) {
        let tasksFiltered = this.tasks.filter((t) => Object.values(t.happens).includes(timelineDate.toString()) || IsRecurringThisDay(t, timelineDate) && !t.done).sort((t) => t, "asc", sortTasks);
        let date = (0, import_obsidian19.moment)(timelineDate.toString()).format(dateFormat);
        let weekday = (0, import_obsidian19.moment)(timelineDate.toString()).format("dddd");
        let year = (0, import_obsidian19.moment)(timelineDate.toString()).format("YYYY");
        let today = (0, import_obsidian19.moment)().format("YYYY-MM-DD");
        let detailsCls = "";
        let content = "";
        let containedTypesPerDay = [];
        if (year != lastYear) {
          containedTypesPerYear = [];
          lastYear = year;
          yearNode = this.rootNode.createEl("div", { cls: "year", attr: { "data-types": "" } });
          if ((0, import_obsidian19.moment)().format("YYYY") == year) {
        ...
        if (timelineDate == today) {
          detailsCls += "today";
          let overdueCount = this.tasks.filter((t) => t.happens["overdue"]).length;
          let todoCount = tasksFiltered.filter((t) => !t.completed && !t.happens["overdue"] && !t.happens["unplanned"]).length;
          let unplannedCount = this.tasks.filter((t) => t.happens["unplanned"]).length;
        ...
        tasksFiltered.forEach((item) => {
          ...
          ;
          let cls = ((_a = Object.keys(item.happens).find((key) => item.happens[key] === timelineDate.toString())) == null ? void 0 : _a.replace("Forward", "")) || "";
          if (cls == "" && item.recurrence) {
            cls = "repeat";
          }
          ;
          ...
          for (let key in item.happens) {
            ...
            let relative = getRelative(value);
            if (!key.includes("Forward") && key != "unplanned") {
              info += `<div class='relative' aria-label='${cls}:${value}'><div class='icon'>${Icons[key]}</div><div class='label'>${relative}</div></div>`;
            }
          }
          if (item.repeat) {
            info += `<div class='repeat' aria-label=''><div class='icon'>${Icons.repeat}</div><div class='label'>${item.repeat.replace("\u{1F501}", "")}</div></div>`;
          }
          ;
        ...
        yearNode == null ? void 0 : yearNode.setAttribute("data-types", containedTypesPerYear.join(" "));
      }
    };
    this.app = app;
    this.dv = (0, import_obsidian_dataview9.getAPI)(app);
  }
};

// src/Tools/POSVaultFunctions.ts
var import_obsidian20 = require("obsidian");
var import_obsidian_dataview10 = __toESM(require_lib());
var POSVaultFunctions = class {
  constructor(app, configFolder) {
    this.setConfigFolder = (configFolder) => {
      this.configFolder = this.app.vault.getAbstractFileByPath(configFolder || "/") instanceof import_obsidian20.TFolder ? configFolder : "";
      if (this.configFolder === "/" || this.configFolder === "")
        this.configFolder = "";
      else if (!this.configFolder.endsWith("/"))
        this.configFolder += "/";
    };
    this.rewardActiveFile = () => {
      var _a;
      ...
      let lists = ((_a = this.dv.page(file.path)) == null ? void 0 : _a.file.lists.where((l) => l.header.subpath && l.header.subpath == "Rewards")) || [];
      const processLinkedFile = async (list) => {
        var _a2, _b, _c;
        if (!list.outlinks || list.outlinks.length !== 1) {
          console.log("No valid outlink found in the task.");
          return;
        }
        let linkedFilePath = list.outlinks[0].path;
        ...
        if (!targetFile || !(targetFile instanceof import_obsidian20.TFile)) {
          console.log(`Target file not found: ${linkedFilePath}`);
          return;
        }
        if (((_a2 = linkedPage.tags) == null ? void 0 : _a2.includes("Competence")) || ((_b = linkedPage.tags) == null ? void 0 : _b.includes("Skill"))) {
          ...
          await this.app.fileManager.processFrontMatter(targetFile, (frontmatter) => {
            let currentLevel = frontmatter["Level"] || 0;
        ...
        } else if ((_c = linkedPage.tags) == null ? void 0 : _c.includes("Achievement")) {
          await this.app.fileManager.processFrontMatter(targetFile, (frontmatter) => {
      ...
      const modifyListItem = async (line) => {
    ...
    this.mapWidget = (dv) => {
      var _a;
      const getTemplatePath = (currentFile2) => {
        var _a2;
        const templateLink = (_a2 = currentFile2["Uses Template"]) == null ? void 0 : _a2.path;
        return templateLink || null;
      };
      const isRequirementMet = (req, headerType) => {
        var _a2, _b;
        const outlink = (_b = (_a2 = req.outlinks) == null ? void 0 : _a2[0]) == null ? void 0 : _b.path;
        const levelProp = req.level;
        if (!outlink)
          return false;
        const targetFile = this.dv.page(outlink);
        if (!targetFile)
          return false;
        if (headerType === "Competences and skills") {
          if (targetFile.tags.includes("Competence") || targetFile.tags.includes("Skill")) {
            const requiredLevel = levelProp || 0;
            const targetLevel = targetFile.Level || 0;
            return targetLevel >= requiredLevel;
          }
        } else if (headerType === "Completed missions") {
          if (targetFile.tags.includes("Mission-Description")) {
            return !!targetFile["Completed on"];
          }
      ...
      const calculateRequirementProgress = (currentFile2) => {
        var _a2;
        const allRequirements = ((_a2 = currentFile2.file.lists) == null ? void 0 : _a2.filter(
      ...
      const isCurrentFileAvailable = (currentFile2) => {
        var _a2;
        const requirements = ((_a2 = currentFile2.file.lists) == null ? void 0 : _a2.filter(
          (list) => ["Competences and skills", "Completed missions", "Obtained achievements"].includes(list.header.subpath)
        )) || [];
      ...
      const isMissionInProgress = (templatePath2) => {
        const referencingFiles = this.dv.pages("#Course OR #Mission OR #Exercise").where((file) => {
          var _a2;
          return ((_a2 = file["From template"]) == null ? void 0 : _a2.path) === templatePath2 && file.Archived !== true;
        });
        return referencingFiles.values.length > 0 ? referencingFiles.values[0] : null;
      };
      const calculateTaskProgress = (inProgressFile) => {
        var _a2, _b, _c, _d;
        const totalTasks = ((_b = (_a2 = inProgressFile.file.lists) == null ? void 0 : _a2.filter((list) => list.task)) == null ? void 0 : _b.length) || 0;
        const completedTasks = ((_d = (_c = inProgressFile.file.lists) == null ? void 0 : _c.filter((list) => list.task && list.completed)) == null ? void 0 : _d.length) || 0;
      ...
      const getUnavailableImage = (currentFile2, templateFile2) => {
        var _a2, _b, _c;
        if (currentFile2.Difficulty > 5) {
          return `${this.configFolder}Images/Mission boss fight image-1.png`;
        } else if ((_a2 = templateFile2.tags) == null ? void 0 : _a2.includes("Mission-Template")) {
          return `${this.configFolder}Images/Mission combat image-1.png`;
        } else if (((_b = templateFile2.tags) == null ? void 0 : _b.includes("Course-Template")) || ((_c = templateFile2.tags) == null ? void 0 : _c.includes("Exercise-Template"))) {
      ...
      if (currentFile && ((_a = currentFile.tags) == null ? void 0 : _a.includes("Mission-Description")) && templatePath) {
        ...
        if (isCompleted) {
          ...
          dv.paragraph(`<h1>${currentFile.file.name}</h1>`);
          ...
          dv.paragraph(`!${currentFile.Image}`);
...
\`\`\`
    ...
    this.getTotalXP = () => calculateTotalPoints(this.dv);
    this.getCurrentLevel = () => Math.floor(currentLevelBasedOnXp(calculateTotalPoints(this.dv), 2, 2));
    this.getXPProgressBar = (dv) => {
    ...
    this.app = app;
    this.dv = (0, import_obsidian_dataview10.getAPI)(app);
    this.setConfigFolder(configFolder);
  }
};

// main.ts
var PersonalOS = class extends import_obsidian21.Plugin {
  constructor() {
    super(...arguments);
    this.manageToggle = () => {
      if (this.settings.toggleXp && !this.xpFeedback) {
        this.xpFeedback = new XpFeedback(this.app);
        this.registerEvent(this.app.metadataCache.on("changed", (f) => this.xpFeedback.displayXpEarned(f, this.profile.display)));
  ...
  async onload() {
    ...
    this.app.workspace.onLayoutReady(async () => {
      ...
      this.registerMarkdownCodeBlockProcessor("Calendar", (source, el, ctx) => {
        ...
        (_a = el.parentElement) == null ? void 0 : _a.classList.remove("markdown-rendered");
      ...
      this.loadCommands();
      this.registerView("Profile View", (leaf) => this.profile = new ProfileView(leaf, this));
    });
  }
};
